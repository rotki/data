...
TEST_ADDR: Final = '0xc37b40ABdB939635068d3c5f13E7faF686F03B65'


def test_airdrops_metadata():
    with open('airdrops/index_v3.json', 'r') as f:
        airdrop_index = json.load(f)
    with open('tests/airdrop_index_v3_schema.json', 'r') as f:
    ...
    for airdrop in airdrop_index['airdrops'].values():
        if 'file_path' in airdrop:
            assert os.path.exists(airdrop['file_path'])
        else:
...
def test_files_and_jsons():
    with open('airdrops/index_v3.json', 'r') as f:
        airdrop_index = json.load(f)

    for airdrop in airdrop_index['airdrops'].values():
        # verify the file_hash is correct
